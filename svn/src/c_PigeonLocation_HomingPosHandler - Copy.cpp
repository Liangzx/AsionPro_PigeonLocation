...
void TCPigeonLocationHomingPosHandler::MsgParsing(const TCString & content, HomingPosPkg & pkg)
{
	try
	{
		...
		pkg.pkg_beijing_time = lsTrackerPkgList[3] == "" ? TCTime::Now(): TCTime::RelativeTime(lsTrackerPkgList[3], 8 * 3600);
	...
	HomingPosPkg & pkg, const int msg_id)
{
	...
	const short tmp_interval = 6;
	if (msg_id == 13) {
		for (int idx = 0; idx < rfid_num; idx++) {
			brgathersubprocinfowithchk.Clear();
			brgathersubprocinfowithchk.homing_time = (char *)(lsTrackerPkgList[10 + tmp_interval * idx].IsEmpty()
				? TCTime::Now() : TCTime::RelativeTime(lsTrackerPkgList[10 + tmp_interval * idx], 8 * 3600));
			brgathersubprocinfowithchk.gps_time = (char *)(lsTrackerPkgList[10 + tmp_interval * idx].IsEmpty()
				? TCTime::Now() : TCTime::RelativeTime(lsTrackerPkgList[11 + tmp_interval * idx], 8 * 3600));
			brgathersubprocinfowithchk.mode_st = (char *)lsTrackerPkgList[12 + tmp_interval * idx];
			brgathersubprocinfowithchk.rfid = (char *)lsTrackerPkgList[13 + tmp_interval * idx];
			brgathersubprocinfowithchk.rfid_chk_1[14 + tmp_interval * idx];
			brgathersubprocinfowithchk.rfid_chk_2[15 + tmp_interval * idx];
		...
		for (int idx = 0; idx < rfid_num; idx++) {
			...
			brgathersubprocinfowithchk.homing_time = (char *)(lsTrackerPkgList[10 + tmp_interval * idx].IsEmpty()
				? TCTime::Now() : TCTime::RelativeTime(lsTrackerPkgList[10 + tmp_interval * idx], 8 * 3600));
			brgathersubprocinfowithchk.gps_time = (char *)(lsTrackerPkgList[11 + tmp_interval * idx].IsEmpty()
				? TCTime::Now() : TCTime::RelativeTime(lsTrackerPkgList[11 + tmp_interval * idx], 8 * 3600));
			brgathersubprocinfowithchk.mode_st = (char *)lsTrackerPkgList[15 + tmp_interval * idx];
			brgathersubprocinfowithchk.rfid = (char *)lsTrackerPkgList[12 + tmp_interval * idx];
			brgathersubprocinfowithchk.rfid_chk_1[13 + tmp_interval * idx];
			brgathersubprocinfowithchk.rfid_chk_2[14 + tmp_interval * idx];
	...
	HomingPosPkg & pkg, const char * video_data, long vedio_size)
{
	...
	std::string sql_buf;
	sql_buf.clear();
	sql_buf = "SELECT SEQ_FOR_VEDIO_ID.NEXTVAL ";
	sql_buf += " FROM DUAL";
	long cur_seq;
	otl_s.open(1, sql_buf.c_str(), m_dbConnect);
	if (!otl_s.eof()) {
	...
	// 入表
	//  video_data
	const int mem_page = 4096;
	otl_long_string nestpad_video(vedio_size + mem_page);	// define long string variable
	m_dbConnect.set_max_long_size(vedio_size + mem_page);	// set maximum long string size for connect object
	otl_s.open(1, sql_buf.c_str(), m_dbConnect);
	//
...
void TCPigeonLocationHomingPosHandler::OutputRpBrPigeonNestVideo(const TCString & pkg_content, HomingPosPkg & pkg)
{
	...
	std::string tb_name = "RP_BR_NESTVIDEO_SUBPROC";
	std::string sql_buf;
	//std::string sequence_name = "SEQ_FOR_VEDIO_ID";
	sql_buf.clear();
	sql_buf = "SELECT SEQ_FOR_VEDIO_ID.CURRVAL ";
	sql_buf += " FROM DUAL";
...
void TCPigeonLocationHomingPosHandler::DoCommand_LocInfo(TCCustomUniSocket  &cusSocket){
	...
	for(int nPkgSeq=0; nPkgSeq<m_vsPkgList.size(); nPkgSeq++){
		...
		if (msg_id == "10") {
			...
			memset(flame_ptr, 0, flame_total_size);
			...
			// 接收相应长度的视频数据
			////视频输出到文件
			for (long len = flame_total_size; len; len = len - rlen)
			{
				rlen = cusSocket.ReceiveBuf(flame_ptr + (flame_total_size-len), len);
			...
			std::fstream iof("./vedio_check.dat", std::ios::ate|std::ios::out|std::ios::binary);
